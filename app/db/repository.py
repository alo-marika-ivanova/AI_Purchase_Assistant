...
class PurchasingRepository:
        ...
        with get_connection() as conn:
            rows = conn.execute(
                ...
                SELECT c.id, c.case_number, c.item_material, c.quantity, c.notes, c.status,
                       c.created_at, c.auto_send_messages, COUNT(cs.supplier_id) AS supplier_count
    ...
    def get_case_details(self, case_id: int) -> dict | None:
        with get_connection() as conn:
            case = conn.execute(
                "SELECT * FROM negotiation_cases WHERE id = ?",
                (case_id,),
            ).fetchone()
    ...
    def get_case_basic(self, case_id: int) -> dict | None:
        ...
        with get_connection() as conn:
            row = conn.execute(
                """
                SELECT id, case_number, item_material, quantity, notes, status, auto_send_messages
...
#        return row is not None
    ...
    def list_cases_for_transport_worker(self) -> list[dict]:
        """
        Return cases that may still need automatic communication processing.
        """
        with get_connection() as conn:
            rows = conn.execute(
                """
    ...
    def acquire_action_lock(
        ...
        """
        Try to create a unique action lock.

        Returns:
            True  = lock created, action may run
            False = lock already exists, action must be skipped
        """
        with get_connection() as conn:
            try:
                conn.execute(
                    """
                    INSERT INTO negotiation_action_locks
                    (
                        case_id,
                        supplier_id,
                        action_key,
